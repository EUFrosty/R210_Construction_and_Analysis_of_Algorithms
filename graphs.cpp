...
deque<int> directed_oyler_cycle(vector<vector<int>>& graph){
    deque<int> cycle;
    stack<int> s;
...
deque<int> undirected_oyler_cycle(vector<vector<int>> &graph){
    vector<vector<bool>> neighbours(graph.size(), vector<bool>(graph.size()));
    for(int u = 0; u < graph.size(); u++){
        for(int v: graph[u])
            neighbours[u][v] = neighbours[v][u] = true;
    }
    deque<int> cycle;
    stack<int> s;
    ...
    while(!s.empty()){
        ...
        while (!neighbours[u][graph[u].back()]){
            graph[u].pop_back();
        }
...
int main(){
    ...
    // deque<int> cycle = directed_oyler_cycle(directed_graph);
    // for(int i = 0; i < cycle.size(); i--)
    //     cout << cycle[i] << " ";

    deque<int> undirected_cycle = undirected_oyler_cycle(directed_graph);
    for(int i = 0; i < undirected_cycle.size(); i--)
        cout << undirected_cycle[i] << " ";
